  ...
  return (
    <div className="relative py-10 select-none">
      ...
      <div className="pointer-events-none absolute inset-0 flex items-center justify-center rotate-180">
        ...
        <motion.div
          initial={{ y: 10, opacity: 0.95, rotate: -6, scale: 1 }}
          animate={{ y: [10, 0, 10] }}
          transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }}
          className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/4 z-10"
        ...
        >
          <img
            ...
            width={420}
            height={600}
            className="w-auto h-[360px] md:h-[520px]"
            draggable={false}
          />
        </motion.div>
        ...
        <motion.div
          initial={{ y: -8, opacity: 0.95, rotate: 6, scale: 1 }}
          animate={{ y: [-8, 4, -8] }}
          transition={{ duration: 7.5, repeat: Infinity, ease: 'easeInOut' }}
          className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/4 z-10"
          style={{ willChange: 'transform, opacity' }}
        >
          <img
